...
private:
    ...
    */
    struct RigidBody
    {
        ...
        std::unique_ptr<btDefaultMotionState> State; ///< Состояние движения для интерполяции движения
        int Mask = NO_MASK;                          ///< Маска столкновения тела
        int Index = 0;                               ///< Внутренний индекс тела в контейнере
        int Shape = 0;                               ///< Тип формы тела
        int Group = 0;                               ///< Группа фильтра столкновений, принадлежащая телу
        int MeshID = 0;                              ///< ID графической сетки для рендеринга
        int MeshInstance = 0;                        ///< Связанный экземпляр графической сетки
        bool ProcessEvents = true;                   ///< Независимо от того, требует ли этот орган проверка столкновения
    };
