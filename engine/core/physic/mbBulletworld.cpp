...
//................................
// Добавить объект в сцену физики
//................................
...
//...........................
// Создание тела ввиде сферы
//...........................
btRigidBody* BulletWorld::addSphere(float rad, float x, float y, float z, float mass, Material spheremat, int spheretag)
{
	btTransform transform;	// положение и вращение
	transform.setIdentity();
	transform.setOrigin(btVector3(x, y, z));	// поместите его в координаты x, y, z
	
	btSphereShape* sphere = new btSphereShape(rad);
	btVector3 inertia(0, 0, 0);	// инерция составляет 0,0,0 для статического объекта, иначе
	
	if (mass != 0.0)
		sphere->calculateLocalInertia(mass, inertia);	// это может быть определено этой функцией (для всех видов фигур)

	btMotionState* motion = new btDefaultMotionState(transform);	//установить положение (и движение)
	btRigidBody::btRigidBodyConstructionInfo info(mass, motion, sphere, inertia);	// создайте buildinfo, вы можете создать несколько тел с той же информацией
	btRigidBody* body = new btRigidBody(info);	// давайте создадим тело
	world->addRigidBody(body);	// и пусть мир узнает об этом
